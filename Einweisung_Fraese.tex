...
\begin{itemize}
	\item Kühlwasserstand Kühlgerät prüfen, ob er zwischen roter und blauer Markierung ist, wenn das Gerät läuft.
...
\begin{itemize}
	...
	\item Druckluftanzeigen kontrollieren (4 bzw. 6 bar)
	\item Kondensatabscheider entleeren: von unten dagegendrücken, bis kurz Luft ausströmt
	\item Kabelketten kontrollieren und ggf. reinigen
	\item Not-Aus und alle 3 Türschalter auf Funktion prüfen
	\item Filter am Kühlgerät prüfen und ggf. reinigen
	\item Lüfter der Steuerung absaugen
...
\begin{itemize}
	...
	\item Spannzange auswählen (Eine Spannzange mit Bezeichnung 3-2 ist für Fräser mit 2\dots 3\,mm Schaft. Ein Fräser mit  3,1\,mm Schaft darf also \emph{nicht} in eine 3-2 Spannzange!)
	\item Spannzange in Spannmutter einsetzen: \emph{Schräg ansetzen} und dann einklicken.
	\item Fräser auf Beschädigung prüfen
	\item Fräser einspannen, möglichst kurz einspannen und die Spannmutter fest anziehen.
\end{itemize}
	\item Werkzeugwechsel (\knopf{User} $\rightarrow$ \knopf{Werkzeugwechsel})
\begin{itemize}
	\item Bisherigen Werkzeughalter auswerfen: Diesen festhalten, damit er dabei nicht herunterfällt. Grünen Taster an Spindel gedrückt halten, Halter entnehmen.
	\item (Für Wartungszwecke kann man hier kurz pausieren und die Wartung durchführen)
	\item Neuer Halter und Gegenstück an der Spindel muss sauber sein (Kegel und Planfläche)!
	\item Neuen Halter einsetzen, dazu wieder Taster gedrückt halten, Halter bis ganz oben einschieben, Taster loslassen
	...
	\item \knopf{OK}, Werkzeugnummer eingeben, \knopf{OK}
	\item Jetzt unbedingt \knopf{Reset} drücken, nichts anderes!
\end{itemize}
	\item Werkzeuglänge messen  (\knopf{User} $\rightarrow$ \knopf{Wkz.länge})
\end{enumerate}
...
Damit die Spannmutter nicht herausfallen kann, beim Vorheizen entweder einen Fräser fest einspannen oder Spannmutter und Spannzange entnehmen.
Während des Vorheizens müssen die Türen geschlossen bleiben. % TODO bei neuer Spindel mit automatischem Wkzw. müssen wir immer einen Werkzeughalter drinhaben.
 \item Kühlung an. Kurz warten, dass die Luftblasen weg sind. Strahlen auf die Fräserspitze richten und mittelmäßig stark einstellen. \\ Zum besseren Ansaugen gegen die ersten Luftblasen gibt es den Boost-Taster. \\ Kommt genug Nebel? (z.B. Acrylplatte in den Nebelstrahl halten und schauen wieviel Tropfen hängenbleiben) 
...
Es geht nur ein Werkzeug pro Datei. Neue Datei laden, Werkzeugwechsel wie oben beschrieben, starten.
...
\begin{enumerate}
 \item \textbf{Beachte:} Die angezeigte Zeit stimmt zur Zeit nicht, wenn man zwischendrin pausiert. \\\todo{Das ist ein Bug, der bereits gemeldet, aber noch nicht behoben wurde.}\\ Bitte orientierte dich an der vorausgesagten Zeit, die nach dem Neuladen der Datei angezeigt wird oder stoppe die Maschinenzeit während des Betriebs.
